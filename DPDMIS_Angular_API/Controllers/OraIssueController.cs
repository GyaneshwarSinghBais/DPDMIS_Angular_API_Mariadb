...
namespace DPDMIS_Angular_API.Controllers
{
    ...
    [ApiController]
    public class OraIssueController : ControllerBase
    {
        //private readonly MariaDbContext _context;
        private readonly OraDbContext _context;
        public OraIssueController(OraDbContext context)
        {
            _context = context;
        }
        ...
        WHERE a.FacilityID = :facid
          AND NVL(a.status,'IN') = 'IN'
          AND a.IssueNo = :Issueno
        ORDER BY a.IssueDate";

                OracleParameter[] parameters =
                {
            new OracleParameter("facid", ffcid),
            new OracleParameter("Issueno", issueno)
        };
        ...
        public string getGeneratedIssueNo(string facId)
        {
            ...
            OraFacOperations op = new OraFacOperations(_context);
    ...
    ORDER BY a.IssueDate";
        ...
        public async Task<ActionResult<IEnumerable<WardIssueItemsDTO>>> getWardIssueItems(string faclityId, string issueId)
        {
            string qry = @"   select mi.ITEMCODE ||'-' || mi.itemname || '-' || to_char( (case when (b.qastatus ='1' or  mi.qctest='N') then (sum(nvl(b.absrqty,0)) - sum(nvl(iq.issueqty,0))) end) ) as name              
                 ...
                 and b.expdate > trunc(sysdate)
                 and t.facilityid= " + faclityId + @"
                 and mi.itemid not in
                     (select distinct itemid
                        from tbfacilityissueitems
                       where issueid = " + issueId + @")
                 and ((case when (b.qastatus ='1' or mi.qctest='N')
    ...
    nvl(mi.multiple,1) as multiple,nvl(INDENTqty,0) as INDENTQTY
                     ...
                     LEFT OUTER JOIN (
                         ...
                         where a.status='C'
                         and b.bookedflag in ('B','C')
                         and b.BookedQty >0
                         and a.NOCID=" + indentid + @"
                         AND B.ITEMID=" + itemid + @"
        ...
        {
            OraFacOperations ObjFacOp = new OraFacOperations(_context);
        ...
        {
            string qry = @" Select IssueItemID,ISSUEID,ItemID
                    from tbFacilityIssueItems
            ...
            if (myList.Count > 0)
            {
                int issueItemID = myList[0].ISSUEITEMID;
        ...
        {
            string WhereConditions = " and (rb.ExpDate >= trunc(sysdate) or nvl(rb.ExpDate,trunc(sysdate)) >= trunc(sysdate)) ";
    ...
    rb.BatchNo,
    NVL(rb.MfgDate, NULL) AS MfgDate,
    NVL(rb.ExpDate, NULL) AS ExpDate,
    NVL(rb.AbsRqty, 0) AS AbsRQty,
    NVL(x.issueQty, 0) AS AllotQty,
    NVL(rb.AbsRqty, 0) - NVL(x.issueQty, 0) AS avlQty,
    rb.Inwno,
    NVL(rb.whinwno, 0) AS whinwno
          ...
          where rb.qastatus=1 and r.status='C' and nvl(rb.whissueblock,0) in (0) and r.FacilityID=" + FacilityID + @"   and i.ItemID= " + ItemID + @"    
           ...
           and Round(rb.ExpDate-trunc(sysdate),0) >= 15 order by expdate";
        ...
        {
            string strQuery1 = "Update tbFacilityIssues set Status='C',IssuedDate=trunc(sysdate) Where IssueID=" + IssueID;
...
left outer join
                ...
                Where 1=1 " + whclause + @" and  T.Status = 'C'
And (b.Whissueblock = 0 or b.Whissueblock is null)
and b.expdate > trunc(sysdate)
    ...
    m.strength1,
    tb.ISSUENO,
    TO_CHAR(tb.ISSUEDDATE, 'DD-MM-YYYY') AS IssueDT,
    tbw.wardname,
    CASE
